...
import { checkLoginUser } from "../../Services/Store/ClientLogin/client.action";
...
const ClientLogin = () => {
  ...
  const loginData = useSelector((state) => state.userClient);

  useEffect(() => {
    if (JSON.parse(sessionStorage.getItem("clientToken"))) {
      setIsLoginStatus(1);
      navigate("/event");
    } else {
      if (loginData && loginData.currentClientUser?.error) {
        setIsLoginStatus(0);
      }
    }
  }, [loginData]);
        ...
        <div className="container">
          <div className="row login-wrap">
            <div className="col-md-7 col-xs-12 admin-image">
              <img
                src="./assets/images/admin-login-panel.jpg"
                alt=""
                className="img-fluid"
              />
            ...
            <div className="col-md-5 col-xs-12">
              <div className="form-content">
                <form onSubmit={(event) => _handleSubmit(event)}>
                  <div className="row">
                    {isLoginStatus === 0 ? (
                      <div className="errorMsg text-danger text-center mb-2">
                        Invalid Credentials
                      </div>
                    ) : (
                      <div className="successMsg text-success"></div>
                    )}
