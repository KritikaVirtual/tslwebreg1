...
export function RegInfoGroup(props) {
  ...
  useEffect(() => {
    ...
    if (lAccountID && lEventID) {
      ...
      if (lRegID) {
        ...
        dispatch(getRegistrantsInformationTemplate21({lAccountID, lEventID, lRegID}))
        dispatch(
  ...
  const getRegSessionData = postData => {
    dispatch(getRegistrantSessionsTemplate21({ 
      lAccountID : postData.lAccountID,
      lEventID : postData.lEventID,
      lRegID : postData.lRegID
    }));
  }

  const getRegistrantInformationByID = postData => {
    dispatch(getRegistrantsInformationByIDTemplate21({ 
      lAccountID : userId,
      lEventID : eventId,
      lRegID : postData
    }));
          ...
          <div className="container-fluid demo">
            <Accordion defaultActiveKey="0">
              <Accordion.Item eventKey="0">
                <Accordion.Header>
                  Main Contact Information ( Access Code : Not Set )
                </Accordion.Header>
                <Accordion.Body>
              ...
              <Accordion.Item eventKey="1">
                ...
                <Accordion.Body>
                  <GuestsAddtionalInformation
                    ...
                    regId={regId}
                    registrantInfo={
                      registrantsData.registrantsInfo
                        ? registrantsData.registrantsInfo.result
                        : []
                    }
                    regTypesData={
                      regTypesData.regScode &&
                    ...
                    getRegistrantInfoByID = {(postData) => getRegistrantInformationByID(postData)}
                    sendRegistrantInfoByID = {registrantsData.registrantsInfoByID
                    ? registrantsData.registrantsInfoByID.result
                    : []}
                    getSessionsData = {(postData) => getRegSessionData(postData)}
                    sendSessionsData = {registrantsData.registrantsSessionsInfo
                        ? registrantsData.registrantsSessionsInfo.result
                        : {}
                    }
              ...
              <Accordion.Item eventKey="5">
                <Accordion.Header>Process Credit Card Payment or Refund</Accordion.Header>
                <Accordion.Body>
                  <ProcessingPayments
                    userId={userId}
                    eventId={eventId}
