...
import { SectionEventInfoBasic } from "../ShortCodes/SectionEventInfoBasic";
import { SectionRegistrantFullInfo } from "../ShortCodes/SectionRegistrantFullInfo";
import { SectionRegistrationType } from "../ShortCodes/SectionRegistrationType";
import { SectionAdditionalRegistrants } from "../ShortCodes/SectionAdditionalRegistrants";
import { SectionGuests } from "../ShortCodes/SectionGuests";
import { SectionSessions } from "../ShortCodes/SectionSessions";
import { SectionTotalWithOtherFees } from "../ShortCodes/SectionTotalWithOtherFees";
import { SectionPaymentAll } from "../ShortCodes/SectionPaymentAll";
    ...
    const emailSetupData = useSelector(emailSetupSelector)
      ...
      const callShortCodes = data => {
        ...
        if(data.includes('[Registrant_First_Name]')){
          result = result.replace('[Registrant_First_Name]',props.sendPersonalInformationValuesSubmitted.sFirstName)
        } 
        if(data.includes('[Registrant_Last_Name]')){
          result = result.replace('[Registrant_Last_Name]',props.sendPersonalInformationValuesSubmitted.sLastName)
        }  
        ...
        if(data.includes('[Section_Additional_Registrants]')){
          const sectionAdditionalRegistrantsHtml = ReactDOMServer.renderToStaticMarkup(<SectionAdditionalRegistrants
            additionalRegistrantData={props.sendPersonalInformationValuesSubmitted} />)
          result = result.replace('[Section_Additional_Registrants]',sectionAdditionalRegistrantsHtml)
        }
        if(data.includes('[Section_Guests]')){
          const sectionGuestsHtml = ReactDOMServer.renderToStaticMarkup(<SectionGuests
            guestsData={props.sendPersonalInformationValuesSubmitted} />)
