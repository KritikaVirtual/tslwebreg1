...
var MultiDayTimelineView = function (props) {
    ...
    var body = (React.createElement(SchedulerResourceIteratorContext_1.SchedulerResourceIteratorContext.Consumer, null, function (_a) {
        ...
        return (React.createElement("div", { className: "k-scheduler-row" }, ranges.map(function (_, rangeIndex) {
            return (slots
                ...
                var utcZonedDayStart = kendo_date_math_1.ZonedDate.fromUTCDate((0, utils_1.toUTCDateTime)(slot.zonedStart), timezone);
                var isWorkDayUTCStart = (0, utils_1.isInDaysRange)(utcZonedDayStart.getDay(), workWeekStart, workWeekEnd);
                return (React.createElement(EditSlot, __assign({ key: "".concat(slot.start.getTime(), ":").concat(slot.group.index), slot: props.slot, viewSlot: props.viewSlot }, slot, { form: props.form, onDataAction: props.onDataAction, isWorkHour: (0, utils_1.isInTimeRange)(slot.zonedStart, workDayStart, workDayEnd), isWorkDay: isWorkDayUTCStart, col: orientation === 'horizontal'
    ...
    var schedulerElement = element.current
        ...
        React.createElement(BaseView_1.BaseView, { ref: element, id: props.id, style: (__assign({}, props.style)), className: className, props: props, slots: slots, ranges: ranges },
            ...
            React.createElement("div", { className: "k-scheduler-body", style: { width: width }, ref: bodyRef },
                ...
                items
                    ...
                    .map(function (item, itemIndex) { return (React.createElement(EditItem, __assign({ key: item.isRecurring
                        ? "".concat(item.uid, ":").concat(item.group.index, ":").concat(item.range.index, ":").concat(item.originalStart)
                        : "".concat(item.uid, ":").concat(item.group.index, ":").concat(item.range.index) }, item, { format: 't', form: props.form, onDataAction: props.onDataAction, item: props.item, viewItem: props.viewItem, editable: props.editable, ignoreIsAllDay: true, vertical: false, isLast: (itemIndex === items.length - 1) }))); })))));
};
